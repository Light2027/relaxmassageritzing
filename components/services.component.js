...
import {GetCommonContent, GetServiceContent, GetServicesContent} from "../services/content.service";
...
export default function ServicesComponent() {
  ...
  const services = GetServicesContent()[locale].services;
  ...
  return <div id="services" className="border-bottom d-flex flex-column justify-content-center">
    ...
    <div className="border border-2 border-secondary bg-white col-11 col-md-6 col-lg-3 align-self-center rounded mt-3 mb-5 p-3">
      {services.map((s) => (
        <p key={s.name} className={`${s !== last ? "border-bottom" : ""}`}>
            ...
            <span>
              ...
              <span className="mx-1"/>
              {new Intl.NumberFormat(content[locale].countryOfCurrency, { style: 'currency', currency: content[locale].currencyType }).format(s.price)}
            </span>
          </p>
      ))}
    </div>
